...
# operform simple image segmentation
# šedá na šedym obrázky, mam určitz co to je za obrázek a kde je (bw maska)
# vastupy maj stejnej formáz jako mnist, hlavně proto že zo zak je jednodušší
# ptro nás; větčí obrázky porevujou větí cpu power, tak je to zas 28x28
# obrázky jsou fashion_mnist
# open data task -- test set emá laveklym nan oredukvoiat nmasjy a klehbky
# je tam nějakej eval skript pro dev dara kterej mui dá accuracy (intersection
# ivetr uunin)
# lower cizubd je prej celjkem low, dát 75% by nemělo bejt těřký
# the template can load tghe datam ur us uin npz format, což jjsou npy files
# zazipovaný do jednoho fajlůu

# mask rcn, predicting bitmask of object: use a series of convolutions, in the
# last laywe for each pixel predict the probability of it belionging to the
# object or not; convolintions work better than fully connected klayers; they
# just use cnns with no stride, stay in 28x28 resolutiion
# prej je lepší mít jinou masku pro každej lejblm, takže poslední cnn vrstva má
# 10 channels, a pro každej lejbl to generuje jinou masku, a podle lejblu pak
# vyberu ten patřiščnej ouput

# baseline: user whareer ui zsed for mnuist compeeteionm, and then add segeral
# layers od xonciolutrions, and then add last conv with 10 output fiolters with
# no activation functionm

# 6 layers of 3x3 convolutions could be enough
...
if __name__ == "__main__":
    ...
    # tohle outputzhe oreductuiobs fir tge test set -- index of class, 0/1
    # bitmas ofthe mask
